...
package net.mcreator.gurkmod.world.dimension;
...
public class FixedPotartzDimension extends GurkmodModElements.ModElement {
	@ObjectHolder("gurkmod:fixed_potartz_portal")
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(Blocks.SMOOTH_QUARTZ);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("gurkmod:fixed_potartz_biome")).getGenerationSettings()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.082352941176, 0, 0.082352941176);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("gurkmod:fixed_potartz"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("fixed_potartz_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("fixed_potartz_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("fixed_potartz_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new FixedPotartzItem().setRegistryName("fixed_potartz"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("fixed_potartz_portal");
		}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("gurkmod:fixed_potartz"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("gurkmod:fixed_potartz")));
				} else {
					entity.func_242279_ag();
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? Blocks.QUARTZ_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.QUARTZ_BLOCK.getDefaultState(), 3);
					}
				}
			}
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getFrom() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("gurkmod:fixed_potartz"))) {

			FixedPotartzPlayerLeavesDimensionProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity))
